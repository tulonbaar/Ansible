# Remove: 
# Older than: 1 months
# If less than 4 folders don't remove
...
if ($directories.Count -gt 0) {
    # Check if the total count of directories is less than 4
    if ($directories.Count -lt 4) {
        Write-Output "Total directory count is less than 4. No directories will be removed."
        return
    }
    ...
    if ($oldestDirectoryDate -lt $twoMonthsAgo) {
        ...
        Write-Output "The oldest directory ($($oldestDirectory.Name)) is less than 1 months old and will not be removed."
    }
} else {
